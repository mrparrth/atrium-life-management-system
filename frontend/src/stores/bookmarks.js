...
export const useBookmarksStore = defineStore('bookmarks', () => {
  ...
  async function updatePage(id, patch) {
    ...
    await db.bookmark_pages.put({ ...p })
  }
  async function removePage(id) {
    // detach bookmarks from this page
    const toDetach = items.value.filter(b => b.pageId === id)
    for (const b of toDetach) { b.pageId = null; await db.bookmarks.put({ ...b }) }
    await db.bookmark_pages.delete(id)
    pages.value = pages.value.filter(p => p.id !== id)
  }

  function bookmarksInPage(pageId) {
    return items.value.filter(b => b.pageId === pageId)
  }
  function looseBookmarks() {
    return items.value.filter(b => !b.pageId)
  }
