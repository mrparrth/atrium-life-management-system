...
<template>
  <div class="px-8 md:px-12 py-10 max-w-6xl mx-auto" data-testid="finance-view">
    <PageHeader overline="Memory · Finance" title="Money, gently tracked" sub="Date-stamped net-worth snapshots and month-by-month cash flow." />
    ...
    <template v-if="tab === 'overview'">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-10">
        <div class="card p-6">
          ...
          <div class="font-serif text-4xl tracking-tight mt-2" data-testid="finance-net-worth">{{ inr(finance.currentNetWorth) }}</div>
          ...
          <div class="mt-6"><Sparkline :data="sparkData" :height="64" color="rgb(90 115 83)" /></div>
          <button class="btn-primary mt-5" @click="openNwForm()" data-testid="overview-log-nw"><Plus class="w-4 h-4" /> Log net worth</button>
        </div>

        <div class="card p-6">
          <div class="overline">Latest month — {{ latestCf ? formatMonth(latestCf.month) : '—' }}</div>
          <div class="grid grid-cols-2 gap-3 mt-3">
            <div><div class="overline">Income</div><div class="font-serif text-lg text-pri-strategic">{{ inr(latestCfTotals.income) }}</div></div>
            <div><div class="overline">Expense</div><div class="font-serif text-lg text-pri-critical">{{ inr(latestCfTotals.expense) }}</div></div>
            <div><div class="overline">Invested</div><div class="font-serif text-lg text-pri-interruptive">{{ inr(latestCfTotals.investment) }}</div></div>
            <div><div class="overline">Net</div><div class="font-serif text-lg" :class="latestCfTotals.net >= 0 ? 'text-ink' : 'text-pri-critical'">{{ inr(latestCfTotals.net) }}</div></div>
          </div>
          <div class="mt-5"><Sparkline :data="cashflowSpark" :height="48" color="rgb(158 132 87)" /></div>
          <button class="btn-primary mt-5" @click="openCfForm()" data-testid="overview-log-cf"><Plus class="w-4 h-4" /> Log a month</button>
        </div>
      </div>
      ...
      <SectionHeader v-if="finance.expenseBreakdownLatest.length" overline="Spending shape" title="Latest month — expenses" />
    ...
    <template v-else-if="tab === 'networth'">
      <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <div class="font-serif text-3xl mt-1">{{ finance.networthLogs.length }} log<span v-if="finance.networthLogs.length !== 1">s</span></div>
        </div>
        <button class="btn-primary" @click="openNwForm()" data-testid="nw-add-btn"><Plus class="w-4 h-4" /> Log net worth</button>
      </div>
        ...
        <div v-for="log in finance.networthLogs" :key="log.id" class="card p-6" :data-testid="`nw-log-${log.id}`">
          <div class="flex items-start justify-between gap-4 mb-4">
            ...
            <div class="flex gap-1">
              <button class="btn-ghost !p-1.5" @click="openNwForm(log)" :data-testid="`nw-edit-${log.id}`"><Edit3 class="w-4 h-4" /></button>
              <button class="btn-ghost !p-1.5 hover:text-pri-critical" @click="deleteNw(log)" :data-testid="`nw-delete-${log.id}`"><Trash2 class="w-4 h-4" /></button>
            </div>
          </div>
          <div class="grid grid-cols-2 md:grid-cols-3 gap-3 pt-4 border-t border-line">
            <div v-for="e in log.entries" :key="e.category + e.type" class="flex items-baseline justify-between">
              <span class="text-xs text-ink-3 capitalize">{{ label(e.category) }}<span v-if="e.type === 'liability'" class="text-pri-critical"> · liability</span></span>
              <span class="text-sm font-medium" :class="e.type === 'liability' ? 'text-pri-critical' : 'text-ink'">{{ e.type === 'liability' ? '-' : '' }}{{ inr(e.value) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <template v-else-if="tab === 'cashflow'">
      <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <div class="font-serif text-3xl mt-1">{{ finance.cashflowPeriods.length }} month<span v-if="finance.cashflowPeriods.length !== 1">s</span></div>
        </div>
        <button class="btn-primary" @click="openCfForm()" data-testid="cf-add-btn"><Plus class="w-4 h-4" /> Log a month</button>
      </div>
          ...
          <div class="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-1 mt-2">
                <div class="flex items-baseline gap-2"><ArrowDownToLine class="w-3 h-3 text-pri-strategic" /><span class="text-ink-2 text-xs">In</span><span class="font-serif text-base text-pri-strategic">{{ inr(finance.periodTotals(p).income) }}</span></div>
                <div class="flex items-baseline gap-2"><ArrowUpFromLine class="w-3 h-3 text-pri-critical" /><span class="text-ink-2 text-xs">Out</span><span class="font-serif text-base text-pri-critical">{{ inr(finance.periodTotals(p).expense) }}</span></div>
                <div class="flex items-baseline gap-2"><PiggyBank class="w-3 h-3 text-pri-interruptive" /><span class="text-ink-2 text-xs">Inv</span><span class="font-serif text-base text-pri-interruptive">{{ inr(finance.periodTotals(p).investment) }}</span></div>
                <div class="flex items-baseline gap-2"><Wallet class="w-3 h-3 text-ink-3" /><span class="text-ink-2 text-xs">Net</span><span class="font-serif text-base">{{ inr(finance.periodTotals(p).net) }}</span></div>
              </div>
              <p v-if="p.note" class="text-sm text-ink-2 italic mt-3">{{ p.note }}</p>
            </div>
            <div class="flex gap-1">
              <button class="btn-ghost !p-1.5" @click="openCfForm(p)" :data-testid="`cf-edit-${p.id}`"><Edit3 class="w-4 h-4" /></button>
              <button class="btn-ghost !p-1.5 hover:text-pri-critical" @click="deleteCf(p)" :data-testid="`cf-delete-${p.id}`"><Trash2 class="w-4 h-4" /></button>
            </div>
          </div>
          <div class="grid grid-cols-2 md:grid-cols-3 gap-3 pt-4 border-t border-line">
            <div v-for="e in p.entries" :key="e.category + e.type" class="flex items-baseline justify-between">
              <span class="text-xs text-ink-3 capitalize">{{ label(e.category) }} <span class="text-[10px] uppercase tracking-overline">·{{ e.type === 'expense' ? 'out' : e.type === 'income' ? 'in' : 'inv' }}</span></span>
    ...
    <template v-else-if="tab === 'categories'">
      <SectionHeader overline="Tags" title="Categories" hint="Net worth categories (asset, liability) and cash flow categories (income, expense, investment) are managed separately." />
      ...
      <div v-for="group in [{label:'Net worth · Assets', scope:'asset'},{label:'Net worth · Liabilities', scope:'liability'},{label:'Cash flow · Income', scope:'income'},{label:'Cash flow · Expenses', scope:'expense'},{label:'Cash flow · Investments', scope:'investment'}]"
