...
<template>
  <div class="px-8 md:px-12 py-10 max-w-7xl mx-auto" data-testid="resources-view">
    ...
    <div v-if="unlinkedNotes.length" class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
      <RouterLink v-for="n in unlinkedNotes" :key="n.id" :to="`/notes/${n.id}`"
        class="card p-5 hover:border-line-2 transition-colors duration-300">
        <div class="flex items-center gap-2">
          <NotebookPen class="w-3.5 h-3.5 text-ink-3" /><span class="overline">Note</span>
        </div>
    ...
    <div v-if="bookmarks.items.length" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <a v-for="b in bookmarks.items" :key="b.id" :href="b.url" target="_blank"
        class="card p-5 hover:border-line-2 transition-colors duration-300 block">
        <div class="flex items-center gap-2">
          <Bookmark class="w-3.5 h-3.5 text-ink-3" /><span class="overline">{{ b.category }}</span>
        </div>
