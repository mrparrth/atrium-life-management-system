...
<template>
  <div class="px-8 md:px-12 py-10 max-w-5xl mx-auto" data-testid="archives-view">
    ...
    <div v-if="archived.length" class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
      <div v-for="p in archived" :key="p.id" class="card p-5 opacity-80">
        <div class="flex items-center gap-2"><FolderKanban class="w-3.5 h-3.5 text-ink-3" /><span class="overline">{{ p.status }}</span></div>
        ...
        <div class="flex items-center justify-between mt-4 pt-3 border-t border-line text-xs">
          ...
          <div class="flex items-center gap-2">
            <button class="btn-ghost text-xs" @click="restoreProject(p)" :data-testid="`restore-${p.id}`">Restore</button>
            <button class="btn-ghost text-xs !text-pri-critical" @click="deleteProject(p)" :data-testid="`delete-${p.id}`"><Trash2 class="w-3 h-3" /></button>
          </div>
        </div>
      </div>
    </div>
