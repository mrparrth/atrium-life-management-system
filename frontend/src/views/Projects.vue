...
<template>
  <div class="px-8 md:px-12 py-10 max-w-6xl mx-auto" data-testid="projects-view">
    <PageHeader overline="PARA · Projects" title="Projects" sub="Things with a finish line.">
      <template #right>
        <button class="btn-primary" @click="showNew = true" data-testid="new-project-btn"><Plus class="w-4 h-4" /> New project</button>
      </template>
    </PageHeader>
    ...
    <div class="flex bg-elevated rounded-xl p-1 border border-line text-sm w-fit mb-8">
      <button v-for="f in ['active','completed','archived','all']" :key="f"
        ...
        @click="filter = f" :data-testid="`project-filter-${f}`">{{ f }}</button>
    </div>
    ...
    <div v-if="filtered.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      <RouterLink v-for="p in filtered" :key="p.id" :to="`/projects/${p.id}`"
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2"><FolderKanban class="w-3.5 h-3.5 text-ink-3" /><span class="overline">{{ areaOf(p.areaId)?.name || 'Project' }}</span></div>
          <span v-if="stale.has(p.id)" class="text-[10px] uppercase tracking-overline text-pri-interruptive">stale</span>
        </div>
        <div>
    ...
    <!-- New project modal -->
    <div v-if="showNew" class="fixed inset-0 z-50 flex items-start justify-center pt-24 px-4" data-testid="new-project-modal">
      ...
      <form @submit.prevent="createProject" class="relative w-full max-w-lg card p-8 animate-rise-in">
        ...
        <input v-model="newTitle" placeholder="Project title…" class="input-soft text-lg font-serif mb-3" required data-testid="new-project-title" />
        <textarea v-model="newDesc" placeholder="A line of context…" rows="2" class="input-soft resize-none mb-3" data-testid="new-project-desc" />
        <div class="grid grid-cols-2 gap-3 mb-5">
          <label class="block"><span class="overline block mb-1">Area</span>
            <select v-model="newArea" class="input-block text-sm"><option :value="null">—</option><option v-for="a in areas.items" :key="a.id" :value="a.id">{{ a.name }}</option></select>
          </label>
          <label class="block"><span class="overline block mb-1">Goal</span>
            <select v-model="newGoal" class="input-block text-sm"><option :value="null">—</option><option v-for="g in goals.items" :key="g.id" :value="g.id">{{ g.title }}</option></select>
          </label>
        </div>
