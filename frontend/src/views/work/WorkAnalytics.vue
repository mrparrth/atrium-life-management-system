...
<template>
  <div class="px-8 md:px-12 py-10 max-w-5xl mx-auto space-y-10 animate-fade-in" data-testid="work-analytics">
    ...
    <!-- HEADER -->
    <PageHeader overline="Intelligence" title="Operational analytics"
      sub="Quiet, strategic insights into client profitability, focus fragmentation, and project margins.">
    </PageHeader>
    ...
    <div class="card p-6 border bg-surface space-y-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <span class="text-[10px] text-ink-3 uppercase tracking-wider font-semibold">Rolling six-month invoiced
            total</span>
        </div>
      ...
      <div class="h-48 flex items-end gap-6 pt-4 px-2 select-none border-b border-line">
        <div v-for="s in revenueSeries" :key="s.key" class="flex-1 flex flex-col items-center h-full justify-end group">
          <!-- Tooltip on hover -->
          <div
            class="opacity-0 group-hover:opacity-100 transition-opacity bg-ink text-canvas text-[10px] px-2 py-1 rounded-md mb-2 font-mono font-semibold shadow-md">
          ...
          <!-- Label -->
          <span class="text-[10px] text-ink-3 font-semibold mt-2 select-none uppercase tracking-wider">{{ s.label
            }}</span>
        </div>
      </div>
    </div>
      ...
      <div class="space-y-6">
        ...
        <div class="card p-6 border bg-surface space-y-4">
          <div class="overline text-ink-3 flex items-center gap-1.5">
            <Activity class="w-3.5 h-3.5" /> Context Switches
          </div>
        ...
        <div class="card p-6 bg-pri-strategic-bg/30 border border-pri-strategic-bd/50 space-y-3">
          <span class="overline text-pri-strategic font-bold flex items-center gap-1.5">
            <Clock class="w-3.5 h-3.5" /> Freelancer Sustainability
          </span>
          <p class="text-xs text-ink-2 leading-relaxed">
            "Sustaining an independent business is not about maximizing short-term hourly output, but maintaining stable
            margins and calm attention boundaries."
          </p>
        </div>
      </div>
    </div>
