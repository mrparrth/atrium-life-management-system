...
export function isConnected() {
  return !!localStorage.getItem('atrium.drive.lastBackup')
}
...
const TABLES = [
  'years', 'goals', 'projects', 'tasks', 'notes', 'bookmarks',
  'areas', 'bookmark_pages', 'next_steps',
  'finance_networth_logs', 'finance_cashflow_periods', 'finance_categories',
  'reviews', 'settings',
]
...
export function downloadLocalBackup() {
  return exportAllData().then(data => {
    const blob = new Blob([JSON.stringify({ schemaVersion: 4, exportedAt: new Date().toISOString(), data }, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url; a.download = `atrium-${new Date().toISOString().slice(0, 10)}.json`; a.click()
