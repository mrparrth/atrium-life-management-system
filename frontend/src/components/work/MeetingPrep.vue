      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
          ...
          <div v-if="clientInfo" class="space-y-1.5 bg-surface/40 p-3.5 rounded-xl border border-line/40">
            ...
            <div class="text-xs text-ink-2">Comm: {{ clientInfo.preferredCommunication }} · {{ clientInfo.timezone }}
            </div>
            <div class="text-[11px] text-ink-3 truncate mt-1">Tech: {{ clientInfo.technicalStack || 'None specified' }}
            </div>
          </div>
            ...
            <ul v-if="activeTasks.length" class="space-y-1">
              <li v-for="t in activeTasks" :key="t.id" class="text-xs text-ink truncate flex items-center gap-1.5">
                <span class="w-1 h-1 rounded-full shrink-0"
                  :class="t.important ? 'bg-pri-critical' : 'bg-ink-3'"></span>
                {{ t.title }}
              </li>
            </ul>
          ...
          <div class="space-y-1.5 bg-surface/40 p-3.5 rounded-xl border border-line/40">
            ...
            <ul v-if="pendingInvoices.length" class="space-y-1">
              <li v-for="i in pendingInvoices" :key="i.id"
                class="text-xs text-ink flex items-center justify-between gap-1.5">
                <span class="truncate">{{ i.invoiceNumber }}</span>
                <span class="font-medium shrink-0"
                  :class="i.status === 'overdue' ? 'text-pri-critical font-semibold' : 'text-pri-interruptive'">
        ...
        <div class="mt-4 flex items-center gap-3">
          ...
          <button v-if="clientInfo" @click="router.push(`/work/clients/${clientInfo.id}`)"
            class="btn-ghost !py-1.5 !px-3.5 text-xs flex items-center gap-1">
            Open workspace
