...
export function todayFocus(tasks) {
  ...
  return tasks.filter(t => isTaskOpen(t) && !isSnoozed(t) && (isToday(t.scheduledDate) || isToday(t.dueDate) || isOverdue(t.dueDate)))
}

export function upcomingTasks(tasks) {
  return tasks.filter(t => isTaskOpen(t) && !isSnoozed(t) && !isToday(t.scheduledDate) && !isToday(t.dueDate) && (isWithinDays(t.scheduledDate, 7) || isWithinDays(t.dueDate, 7)))
}

export function recentlyIgnored(tasks) {
  // open tasks not viewed in 7+ days, no scheduled date today
  return tasks.filter(t => isTaskOpen(t) && !isSnoozed(t) && daysSince(t.lastViewedAt) >= RESURFACE.taskIgnoredDays && !isToday(t.scheduledDate))
}
...
export function staleProjects(projects, tasks) {
  return projects.filter(p => p.status !== 'archived' && p.status !== 'completed' && daysSince(p.lastViewedAt) >= RESURFACE.projectStaleDays)
